...
long Stack_t::calc_sum (char* sum_data, long size)
	{
        ...
        {
            if ( sum_data + i >= (char*)&(this->struct_sum) && sum_data + i <= ((char*)&(this->struct_sum) + sizeof(long) - 1) )
            temp = *(sum_data+i) & 0xFFAAFAFA;
...
elem_t* Stack_t::stackmem_check_allocation ()
	{
		...
		if (size > max_size)
		{
			...
			new_data[max_size+1] = canary4_value;
			return new_data;
		}
...
bool push_test1 ()
{
	...
	{
		if ( ( error = test_stk.push ((elem_t)i) ) != OK)
		{
	...
	if (int size = test_stk.tell_size () != 10)
	{
		...
		test_stk.diagnostic (error);
		return false;
	}
    for (int i = 100; i >= 10; i -= 10)
	{
        if ( ( error = test_stk.pop (pop_value) ) != OK)
		{
            printf ("FIRST TEST FAILED. PUSH IS NOT CORRECT\n");
            test_stk.diagnostic (error);
            return false;
